...
export class Game extends Phaser.Scene {
    ...
    solicitarMovimientoObjetivo(elemento, x, y, angulo) {
        ...
        const enviado = this.network.moverElemento(
            pendingRequest.elementId,
            pendingRequest.x,
            pendingRequest.y,
            pendingRequest.z,
            pendingRequest.angulo
        );
    ...
    // solicitarLanzarDron() removed - unused legacy code (LANZAR_DRON message type not in protocol)
    ...
    solicitarDesplegarDron() {
        ...
        const enviado = this.network.desplegarDron();
        if (enviado) {
    ...
    emitirDisparo(pointer) {
        ...
        const enviado = this.network.disparar(Number(dronId));
    ...
    solicitarRecargaDron(portadronId) {
        ...
        const enviado = this.network.solicitarRecargar(
            Number(dronId),
            Number(portadronId)
        );
    ...
    enviarMovimiento(elemento) {
        ...
        this.network.moverElemento(
            Number(elementId),
            Number(elemento.x),
            Number(elemento.y),
            z,
            Math.floor(elemento.angle || 0)
        );
    }
